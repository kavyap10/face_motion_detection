...
int main(int argc, char** argv)
{
	...
	face_cascade.load("/home/pi/opencv-3.2.0/data/haarcascades/haarcascade_frontalface_alt2.xml");
...
int main(int argc, char** argv)
{
	...
	CascadeClassifier face_cascade;
	face_cascade.load("/home/pi/opencv-3.2.0/data/haarcascades/haarcascade_frontalface_default.xml");
	...
	for (;;)
	{
		...
		for (int i = 0; i < faces.size(); i++)
		{
			Point center(faces[i].x + faces[i].width*0.5, faces[i].y + faces[i].height*0.5);
			//ellipse(img, center, Size(faces[i].width*0.5, faces[i].height*0.5), 0, 0, 360, Scalar(255, 0, 255), 4, 8, 0);
			rectangle(img, faces[i], Scalar(0, 255, 0), 2, 1);
			cout << "1";
		}
